     ...
     if($action->from == '858' AND preg_match("/Anda mendapatkan penambahan pulsa/i", $isi_pesan)) {
         ...
         if (mysqli_num_rows($CheckHistory) == 0) {
             ...
             while($DataDeposit = mysqli_fetch_assoc($CheckHistory)) {
                        ...
                        if($cekpesan == true) {                              
                            $update_history_topup = $conn->query("INSERT INTO riwayat_saldo_koin VALUES ('', '$user', 'Saldo', 'Penambahan Saldo', '$saldo', 'Mendapatkan Saldo Melalui Isi Saldo Via Transfer Pulsa Telkomsel Dengan Kode Isi Saldo : $kode', '$this_date', '$time')");                            
                            $update_history_topup = $conn->query("UPDATE deposit SET status = 'Success' WHERE kode_deposit = '$kode'");
                            $update_history_topup = $conn->query("UPDATE users SET $tipe = $tipe+$saldo WHERE username = '$user'");
